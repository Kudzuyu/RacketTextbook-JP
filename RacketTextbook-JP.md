...
文章中にでてくる練習問題は、章末に解答を載せてあります。全てやりきれば、Racketで書かれたソースコードはほぼとまどうことなく読めるようになり、自分自身でもプログラミングがでるようになるでしょう。
...
Racketの世界では、全てのプログラムおよびデータは()-丸カッコでくくられます。これをリストといいます。Racketの世界では、プログラムとプログラムで扱うデータを同じようにリストで表現します。このうち、プログラムとしてのリストは、先頭の要素および残りの要素に大別されます。このプログラムでいうと、「display」が先頭の部分にあたり、「"Hello World!"」が残りの部分にあたります。プログラムとしてのリストの先頭と残りには、それぞれ役割があります。先頭の要素がプログラムの名前であり、大まかな動作を決定します。残りの要素は「引数(ひきすう)」と呼ばれ、そのプログラムの動作を細かく決定するものです。
この場合「display」は「画面に引数にある文字を出力せよ」という命令になります。従って、引数である「Hello World」が出力されます。
...
### 評価
ここまで見てきたリストは、いづれも「データとしてのリスト」でした。Cや
...
(display '(1 2))
```
このプログラムを実行すると、エラーが出ずに、(1 2)と出力されます。実は、リストの前に'(クオート)を付けるとRacketはそのリストを単なるデータとみなし、評価をしないのです。データとてのリストと、関数としてのリストはこのようにして分けられているのです。
...
コメントは多すぎるとプログラムが逆に見づらくなってしまいます。実際には上のようなプログラムでコメントは必要ないでしょう。
...
'(cat (mouse rabbit))
```
このようになりました。リストが多層構造になっている時に、carやcdrをとると、リストの中のリストは、アトムと同じように扱われます。(car '((a b) c))が、aではなく'(a b)となる事に注意してください。
...
では、次のようなリストを考えてみましょう。 食品の名前と、その値段のリストになります。
```racket
...
1
```
これは数学的な考えにもとづいています。詳しい事は数学の参考書などを参照してください。  
...
225
```
ちゃんと答えが返ってきました。racketでは、ユーザーの作った関数はもとからある関数と同じように使うことができます。したがって、関数の入れ子を、ユーザーの作った関数と基本関数で作ることもできます。
```racket
>(square (car '(11 12 13)))
121
```
...
ところで、シェルでdefineしたときに、これまでと違うことがあることに気付かれたでしょうか。これまでの関数は、即時結果を返すものでしたがdefineはなにもかえってきていません。実はdefineは実行されても値を返さないのです。
...
繰り返しのプログラムを書くには、再帰というテクニックを使うか、forやdoという関数を使います。多くのLisp、Schemeの本やサイトでは再帰を先に解説することが多いのですが、理論としてわかりやすいのはforやdoなので、ここではforを先に解説することにします。forの形は次のようになります。(注:forはRacket独自の関数で、schemeにはありません。schemeではdoや再帰を使ってください)
```rakcet
(for ((変数1)(変数2) ...)
	(動作))
```
...
正直説明が難しいので、この文章だけで理解できる人はいないと思います。実際のプログラムを見ながら理解していきましょう。
...
```racket
(hellos2 4)
->(display "hello")(hellos2 3)
->(display "hello")(display "hello")(hellos2 2)
```
...
(define (last ls)
  (if (null? (cdr ls))
		...
		(last (cdr ls))))
```
この関数では、関数の中で自らを呼びだすということをしています。最初のうちは違和感を感じるかもしれませんが、再帰を使いこなせるようになるとプログラムをforやdoを使った繰り返しよりも簡単にかくことができるようになりますので、是非覚えてください。

続いて、再帰定義を習うときによく使われる、階乗の関数を作ってみましょう。
階乗は $n! = n * (n - 1) * (n - 2) ... * 2 * 1$であらわされます。1つづつ数が小さくなっていくところなどは再帰にピッタリでしょう。

[^fact]:factrial(階乗)の省略形です。

では、プログラムの構造を考えてみましょう。このプログラムでは、再帰の度に数を1つずつ変化させ、それを掛けあわせて階乗を求めていけばよさそうです。以下が階乗の関数になります[^fact]。
...
(define (fact x)
  (if (=> x 0)
    ...
    (* x (fact (- x 1)))))
```
いかがでしょうか。last関数とは最後の処理が少し異なりますが、一つずつリストや数を動かしていくところは同じです。これを動かすと以下のようになります。
